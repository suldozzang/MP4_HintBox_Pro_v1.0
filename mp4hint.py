...
"""
MP4 HintBox Pro - Parallel/Serial selectable processing (Improved v1.0.5)

Refactored from mp4hint_1.0.5.py to allow choosing between:
- Serial processing (original ProcessingWorker using QThread)
- Parallel processing (ProcessWorker using QRunnable + QThreadPool)

Save this file as mp4hint_improved.py and run with a Python environment
that has PyQt5, psutil, and ffmpeg/MP4Box available in PATH.
...
from PyQt5.QtWidgets import (
    ...
    QTextEdit, QSplitter, QCheckBox, QListWidgetItem
)
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QThreadPool, QRunnable, QObject, QUrl, QMutex, QMutexLocker
        ...
        try:
            ...
            if success and os.path.exists(temp_path):
                try:
                    ...
                    self.signals.log.emit(f"파일 이동 실패: {e}")
                    self.signals.result.emit(filepath, "파일 이동 실패 ❌")
                    self.signals.finished.emit(filepath, False)
                    if os.path.exists(temp_path):
                        try:
                            os.remove(temp_path)
...
class MP4ProcessorApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        website_label = QLabel('<a href="https://suldo.com">Official Website</a>')
        website_label.setOpenExternalLinks(True)
        website_label.setAlignment(Qt.AlignLeft)
        bottom_layout.addWidget(website_label)
        
        info_label = QLabel("MP4 HintBox Pro v1.0.5 © suldo.com")
    ...
    def on_files_dropped(self, file_paths: List[str]):
        ...
        for filepath in file_paths:
            is_duplicate = any(
                self.file_list.item(i).text().split(" | ")[0] == filepath 
    ...
    def add_file(self, filepath: str):
        ...
        worker.signals.result.connect(self.update_file_status_from_check)
        worker.signals.error.connect(self.handle_check_error)
        
        self.threadpool.start(worker)
    
    def update_file_status_from_check(self, filepath: str, status: str):
        """CheckWorker에서 호출되는 상태 업데이트"""
        for i in range(self.file_list.count()):
    ...
    def handle_check_error(self, filepath: str, error: str):
        self.update_file_status_from_check(filepath, "체크 실패")
    ...
    def process_all(self):
        ...
        filepaths = [self.file_list.item(i).text().split(" | ")[0] 
                    for i in range(self.file_list.count())]
        self.start_processing(filepaths)
    ...
    def start_processing(self, filepaths: List[str]):
        ...
        if self.config.PROCESS_MODE == "serial":
            ...
            self.log(f"직렬 처리 시작: 전체 {len(filepaths)}개 파일 (처리 대상: {len(files_to_process)}개), "
                    f"엔진: {self.engine_combo.currentText()}, 가속: {self.hw_accel_combo.currentText()}")
            ...
            for filepath in files_to_process:
                ...
                self._parallel_workers.append(pw)
                self.threadpool.start(pw)
            
            self.log(f"병렬 처리 시작: 전체 {len(filepaths)}개 파일 (병렬 대상: {len(files_to_process)}개), "
                    f"최대 동시 작업: {self.threadpool.maxThreadCount()}, "
                    f"엔진: {self.engine_combo.currentText()}, 가속: {self.hw_accel_combo.currentText()}")
    ...
    def handle_parallel_finished(self, filepath: str, success: bool):
        with QMutexLocker(self._parallel_mutex):
            ...
            if self._parallel_done >= self._parallel_total:
                ...
                if self._parallel_errors == 0:
                    QMessageBox.information(
                        self, "완료", 
                        f"병렬 처리 완료: 성공 {self._parallel_success}개, 실패 {self._parallel_errors}개"
                    )
                else:
                    QMessageBox.warning(
                        self, "완료", 
    ...
    def cancel_processing(self):
        ...
        # 병렬 처리 취소 (진행 중인 작업은 완료될 때까지 계속됨)
        if self._parallel_workers:
            for worker in self._parallel_workers:
                worker.stop()
            self.log("병렬 처리 취소 요청됨 (진행 중인 작업은 계속 실행됨)")
